...
### With default rule (regra padrão)

- **Priority**: Regra padrão (`.cursor/rules/default.mdc`) > intelligent-routing
- If the default rule specifies explicit routing, follow it
...
Add to the default rule temporarily (e.g. in `.cursor/rules/default.mdc`):
...
**Next Steps**: Integrate this skill into the default rule (TIER 0) in `.cursor/rules/default.mdc`.
